...
`KDFS Challenge 2023` 일반 트랙에 `정소민진짜예쁘더라` 팀으로 참여하여, `1위(대상/경찰청장상)`을 수상하였다. 1년이나 지나 후기를 업로드하기에는 다소 늦은 감이 있지만, `KDFS Challenge 2024`가 시작하기 전에 간략히 작성해 보겠다.
...
`2023`년에는 `김영끌 랜섬웨어 유포 사건` 이 출제되었다. `Windows PC 포렌식` 과 `암호화폐 트랜잭션 추적` 이 요구되었다. 
...
보고서 작성 시에는 두괄식 작성으로 분석 결과를 빠르게 제시하고, 세부 사항~~과 점수를 위한 세부세부사항~~을 함께 후술하였다. 또한 보고서 말미에 용의자의 행위로 인해 성립되는 범죄가 무엇인지와 고려되는 법정형의 수준에 대해 기술하고, 범죄 수익 몰수 및 공범자 신원 확보를 위한 추가 조치를 다루어 결과물의 차별성을 가져가 보고자 했다. 문제 범위 밖의 사족일 수 있어 실제로 추가 점수를 얻는데 성공하였는지는 잘 모르겠다.
...
우리는 암호화폐 트랜잭션 분석 `Tool`이 없었기 때문에, 일시적인 라이센스를  몇 군데 신청하였지만 받아주는 곳이 없었다. 그래도 [Blockchain.com](https://www.blockchain.com/explorer) 같은 곳에서 검색하는 것만으로 분석할 수 있는 수준이었다. 휴리스틱을 활용한 다음 원칙에 입각하여 분석했다.
